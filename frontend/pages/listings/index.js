...
const loadData = async () => {
    try {
        ...
        for (const listing of listings) {
            html += `<tr>
                <td>${listing.id}</td>
                <td>${listing.title}</td><td>${listing.description}</td>
                <td>${listing.price}</td>
        ...
        document.querySelectorAll('.delete').forEach(btn => {    //ลบ
        ...
        document.querySelectorAll('.edit').forEach(btn => {     //แก้ไข
            btn.addEventListener('click' , async (event) => {
                const id = event.target.dataset.id
                ...
                document.getElementById('button-save').onclick = async () => {       //บันทึก
                    const data = {
                    ...
                    try {
                        await api.listings.update(id,data)                            //  update
                        document.getElementById('popup').style.display = 'none'
                        await loadData()
                    } catch (error) {
                        console.log(error)
