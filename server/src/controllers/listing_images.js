...
const create = async (req, res, next) => {
    try {
        ...
        const { listing_id, image_url, image_order } = req.body
        const errors = []
        ...
        const result = await Listing_imagesModel.create(req.body)
...
const remove = async (req, res, next) => {
    try {
        ...
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'listing_images' })
        }
